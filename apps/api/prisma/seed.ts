...
async function main() {
  ...
  // 1. Clean up existing data (optional / safe)
  // Disable constraint checks first if needed, but simple delete works if ordered correctly
  ...
  for (const p of platformList) {
    platforms[p.slug] = await prisma.platform.create({ data: p })
  }
  ...
  // 7. Create Reviews & Ratings
  console.log('✍️ Creating reviews and ratings...')
  // Elden Ring Reviews
