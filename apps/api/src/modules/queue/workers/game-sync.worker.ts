...
export class GameSyncWorker extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `📥 Starting game-sync job: [${job.name}] | Job ID: ${job.id} | Attempt: ${job.attemptsMade + 1}`,
    )
    ...
    try {
      switch (job.name) {
        ...
        case SYNC_POPULAR_GAMES_JOB: {
          ...
          this.logger.log(
            `🔥 [CorrelationID: ${job.id}] Syncing popular games (Limit: ${finalLimit})`,
          )
      ...
      if (job.attemptsMade + 1 >= maxAttempts) {
        this.logger.warn(
          `⚠️ Job ${job.id} permanently failed. Enqueueing to Dead Letter Queue (DLQ).`,
        )
        try {
